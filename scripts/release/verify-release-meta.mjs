...
if (!versionRegex.test(changelog)) {
  throw new Error(
    `CHANGELOG.md must include a dated section for package version ${version}.`,
  );
}
...
if (tagName) {
  ...
  if (tagName !== expectedTag) {
    throw new Error(
      `Tag/version mismatch: tag=${tagName}, expected=${expectedTag}.`,
    );
  }
}

console.log(
  `release metadata verified for version ${version}${tagName ? ` (${tagName})` : ""}`,
);
