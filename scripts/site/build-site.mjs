...
const docPages = [
  ...
  { source: 'docs/getting-started.md', output: 'docs/getting-started.html', label: 'Getting Started' },
  { source: 'docs/production-checklist.md', output: 'docs/production-checklist.html', label: 'Production Checklist' },
  { source: 'docs/release.md', output: 'docs/release.html', label: 'Release Process' },
  { source: 'examples/README.md', output: 'docs/examples.html', label: 'Examples Guide' },
  { source: 'CONTRIBUTING.md', output: 'docs/contributing.html', label: 'Contributing' },
...
const resolveMarkdownHref = (href, sourcePath, outputPath, exampleOutputBySource) => {
  if (!href) return href
  if (/^(https?:|mailto:|tel:|#)/i.test(href)) return href

  const { base, hash } = parseLinkTarget(href)
  const sourceDir = path.posix.dirname(normalize(sourcePath))
  const normalizedTarget = normalize(path.posix.normalize(path.posix.join(sourceDir, base)))
  ...
  if (fssync.existsSync(absoluteTarget)) {
    ...
    return appendHash(`${githubRepoUrl}/${mode}/main/${normalizedTarget}`, hash)
  }
...
const formatInline = (line, context) => {
  ...
  next = next.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_match, label, href) => {
    ...
    const resolved = resolveMarkdownHref(href, context.sourcePath, context.outputPath, context.exampleOutputBySource)
...
const renderMarkdown = (markdown, context) => {
  ...
  const flushCode = () => {
    const classAttr = codeLanguage ? ` class="language-${escapeAttr(codeLanguage)}"` : ''
  ...
  for (const rawLine of lines) {
    ...
    if (headingMatch) {
      ...
      html.push(`<h${level} id="${escapeAttr(headingId)}">${formatInline(headingText, context)}</h${level}>`)
      continue
    }
    ...
    if (/^[-*]\s+/.test(line)) {
      ...
      html.push(`<li>${formatInline(line.replace(/^[-*]\s+/, ''), context)}</li>`)
      continue
    }
    ...
    if (/^\d+\.\s+/.test(line)) {
      ...
      html.push(`<li>${formatInline(line.replace(/^\d+\.\s+/, ''), context)}</li>`)
      continue
    }
...
const renderLayout = ({ title, outputPath, activeKey, content }) => {
  const navItems = [
    ...
    { key: 'getting-started', label: 'Getting Started', output: 'docs/getting-started.html' },
    { key: 'examples', label: 'Examples', output: 'examples/index.html' },
    { key: 'production', label: 'Production', output: 'docs/production-checklist.html' },
  ...
  const repoLink = '<a class="repo" href="https://github.com/kasparsj/hydra-three">GitHub</a>'
...
const buildDocPages = async (exampleOutputBySource) => {
  for (const page of docPages) {
    ...
    const sidebar = docPages
      .map((doc) => {
        const href = relativeHref(normalize(page.output), normalize(doc.output))
    ...
    const html = renderLayout({
      title: page.label,
      outputPath: page.output,
      activeKey: page.output.includes('examples') ? 'examples' : (
        page.output.includes('release') ? 'release' : (
          page.output.includes('production') ? 'production' : (
            page.output.includes('getting-started') ? 'getting-started' : 'overview'
          )
        )
      ),
      content
...
const createExampleOutputMap = (exampleSources) => {
  ...
  for (const source of exampleSources) {
    const output = normalize(source.replace(/^examples\//, 'examples/').replace(/\.js$/, '.html'))
...
const renderExamplesIndex = async (exampleSources, outputBySource) => {
  ...
  for (const source of exampleSources) {
    ...
    const category = relative.includes('/') ? relative.split('/')[0] : 'general'
