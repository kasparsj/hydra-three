...
// additive animation
plineloop([300], solid(sin(1).add(sin(()=>time/2%1000)).mult(0.5).add(0.5), cos(1).add(cos(()=>time/2%1000)).mult(0.5).add(0.5))).setAutoClear().out();

// dancing thread
plineloop([100], solid(noise(1).x, noise(2).y, noise(3).y).map(-1,1,0,1).out(o1))
    .setAutoClear()
    .out();
